...
const AdminPage = () => {
  ...
  const extractError = (err: any) => {
    ...
    if (err.errors?.length) return err.errors.map((e: any) => e.msg || e.message).join(", ");
  ...
  const handleUpdateBlog = async (blog: any) => {
  ...
  const handleUpdateAnnouncement = async (a: any) => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-10 bg-white shadow-lg rounded-lg my-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/" className="text-[#001A78] underline">
          Back to site
        </Link>
      </div>
